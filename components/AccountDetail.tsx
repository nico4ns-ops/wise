...
const AccountDetail: React.FC<AccountDetailProps> = ({ 
  ...
  // Simple grouping for display (In real app would be dynamic dates)
  const pending = accountTransactions.filter(t => t.status === TransactionStatus.PENDING);
  const completed = accountTransactions.filter(t => t.status !== TransactionStatus.PENDING);
  ...
  return (
    <div className="animate-in fade-in slide-in-from-right-4 duration-300">
      ...
      <div className="mb-8">
        ...
        <div className="inline-flex items-center gap-2 bg-wise-bg px-4 py-2 rounded-full text-sm font-semibold text-wise-dark cursor-pointer hover:bg-green-100 transition-colors">
